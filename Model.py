...
def main():
    ...
    model.add(Bidirectional(LSTM(
            ...
            activation='tanh', return_sequences=True), input_shape=(20, 81)))
    model.add(Bidirectional(LSTM(
            81, kernel_regularizer=regularizers.l2(L2),
            recurrent_regularizer=regularizers.l2(L2),
            bias_regularizer=regularizers.l2(L2), dropout=DROPOUT,
    ...
    check = ModelCheckpoint('wide_samples.{epoch:02d}-{val_loss:.2f}.hdf5', period=20)
    model.compile(loss='categorical_crossentropy',
                  optimizer=optimizers.Adam(lr=0.00001), metrics=['accuracy'])
    model.fit_generator(generator=dg, steps_per_epoch=25, epochs=500, verbose=2, callbacks=[check],
